...
\subsection{proc. ordenarRegionYTipo}
    \subsubsection{Especificaci\'on:}    
        begin{proc}{ordenarRegionYTipo}{\Inout th: $eph_{h}$, \Inout ti : $eph_{i}$}{}
            \pre{sonEncuestasValidas(th,\ ti)\ \y\ th = th_{0}\ \y\ ti = ti_{0}}
            \post{tablasNoCambian(th,\ th_{0},\ ti,\ ti_{0})\ \yLuego\ tablasOrdenadas(th,\ ti)}
        \end{proc}
        % === Requisitos === %
    ...
    \subsubsection{Predicados y funciones auxiliares:}
        \noindent\pred{tablasNoCambian}{th: $eph_{h}$, $th_{0}$: $eph_{h}$, ti: $eph_{i}$, $ti_{0}$: $eph_{i}$}{\\
            \tab tablaHogaresNoCambia(th,\ th_{0})\ \y\ tablaIndividuosNoCambia(ti,\ ti_{0})\\
        }
        $\newline$
        %Revisar 
        ...
        $\newline$
        \pred{tablasOrdenadas}{th: $eph_{h}$, ti: $eph_{i}$}{\\
