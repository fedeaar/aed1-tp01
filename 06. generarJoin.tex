...
\subsection{proc. generarJoin}
    \subsubsection{Especificaci\'on:}
        \begin{proc}{generarJoin}{\In th: $eph_{h}$, \In ti : $eph_{i}$, \Out junta: joinHI}{}
            \pre{sonEncuestasValidas(th,\ ti)}
            \post{|junta| = |ti|\ \yLuego\ duplasValidas(th,\ ti,\ junta)}
        \end{proc}
        % === Requisitos === %
    ...
    \subsubsection{Predicados y funciones auxiliares:}
        \noindent\pred{duplasValidas}{th: $eph_{h}$, ti: $eph_{i}$, junta: joinHI}{\\
    ...
    \subsubsection{Observaciones:}
        \begin{itemize}
            \item el predicado $hayDuplaParaTodoIndividuo$ verifica tanto que est\'en todos los individuos de $ti$ en $junta$ como que no hayan repetidos. Esto es as\'i porque
            previamente se comprueba que $|junta| = |ti|$.
        \end{itemize}
